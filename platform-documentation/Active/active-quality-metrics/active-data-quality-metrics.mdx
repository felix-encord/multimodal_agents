...
1. [K-means clustering](https://en.wikipedia.org/wiki/K-means_clustering) is applied to image embeddings. The total number of clusters is obtained from the Ontology file (if there are both object and image-level information, total object classes are determined as the total cluster number). If no ontology information exists, _K_ is determined as 10.
