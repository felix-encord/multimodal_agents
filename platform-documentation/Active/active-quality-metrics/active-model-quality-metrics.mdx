...
Rank images by their least confidence score. Least confidence takes the difference between 1 (100% confidence) and the most confidently predicted label for each item. It's useful to convert the uncertainty scores to a 0–1 range, where 1 is the most uncertain score.
...
The metric calculates the maximum confidence score for each class within every object-level prediction, averages these across all object predictions within an image, and assigns this average as the image's score. In the absence of any predictions for an image, a score of zero is assigned.